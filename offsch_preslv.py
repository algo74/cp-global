...
def _cp_scheduling_attempt(max_nodes, queue, objective_function='AWF', timelimit=100, verbosity='Normal'):
    ...
    for _, job in squeue.iterrows():
      ...
      submit = mdl.interval_var(start=submit_time, size=1, optional=False,
                                    name='F{}'.format(job_id))
      ...
      queued_job_dict[job_id] = (job, requirements, submit, duration, interval)
    ...
    if objective_function == 'AWF':
      ...
      job_contributions = [nodes * duration * (mdl.start_of(interval) - mdl.start_of(submit))  ## optimized AWF
      ...
      job_contributions = [(mdl.end_of(interval) - mdl.start_of(submit))
               for job, nodes, submit, duration, interval in queued_job_dict.values()]
      objective_var = mdl.sum(job_contributions)
    elif objective_function == 'BSLD':
      job_contributions = [mdl.max(1, (mdl.end_of(interval) - mdl.start_of(submit)) / float(max(10, duration)))
               for job, nodes, submit, duration, interval in queued_job_dict.values()]
      objective_var = mdl.sum(job_contributions)
      ...
      M3 = []
      for job, nodes, submit, duration, interval in queued_job_dict.values():
        Tw = mdl.start_of(interval) - mdl.start_of(submit)
    ...
    for solution in solver:
      ...
      res = squeue
      for job_id, (job, requirements, submit, duration, interval) in queued_job_dict.items():
        job['Wait Time'] = solution.get_var_solution(interval).get_start() - job['Submit Time']
        now = time.time()
...
def offline_scheduling(in_swf_filename, out_swf_filename):
    ...
    result = _cp_scheduling_attempt(numNodes, df, objective_function='P2SF', timelimit=1000, verbosity='Verbose')
...
if __name__ == "__main__":
    ...
    out_swf_filename = "results/tmp.swf"
