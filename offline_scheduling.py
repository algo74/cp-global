...
def _cp_scheduling_attempt(max_nodes: int, queue: pd.DataFrame, objective_function='AWF', timelimit=100, verbosity='Normal'):
    """
    ...
    if res:
      ...
      for job_id, (job, requirements, submit_time, duration, interval) in queued_job_dict.items():
        job['Wait Time'] = res.get_var_solution(interval).get_start() - submit_time
      return squeue
    else:
...
def offline_scheduling(in_swf_filename: str, out_swf_filename: str):
    """
    ...
    result = _cp_scheduling_attempt(numNodes, df, objective_function='AWF', timelimit=1000, verbosity='Verbose')

    ## save the solution adding "; MaxProcs: <numNodes>" to the header
    if result is not None:
        with open(out_swf_filename, 'w') as f:
            f.write("; MaxProcs: {}\n".format(numNodes))
            result.to_csv(f, sep='\t', index=False, header=False)
        # result.to_csv(out_swf_filename, sep='\t', index=False, header=False)
